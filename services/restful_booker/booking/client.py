...
class BookingClient(BaseAPI):
    ...
    def remove(self, booking_id: int, **kwargs) -> tuple[Response, None]:
    ...
    def list(self, params: dict | None = None) -> tuple[Response, list[dict]]:
    ...
    def update(self, booking_id: str | int, validated: BookingData | UpdateBookingPatchRequest,
        ...
        if method.lower() == "put":
            ...
            raise ValueError(f"Некорректный http-метод: {method}. Допустимые значения: 'PUT', 'PATCH'")
        body = BookingData(**response.json()) if validate else response.text
        return response, body
